...
func RegisterRoutes(articleHandler *ArticleHandler, articleTypeHandler *ArticleTypeHandler, api huma.API) {
	// 1. 文章相关的路由组
	articleGroup := sharedApi.NewGroup(api, "/article", "article")
	...
	huma.Delete(articleGroup, "/{id}", articleHandler.Delete, operation("remove", "删除"))

	// 2. 文章分类 相关的路由组
	articleTypeGroup := sharedApi.NewGroup(api, "/article-type", "article-type")
	...
	huma.Delete(articleTypeGroup, "/{id}", articleTypeHandler.Delete, operation("remove", "删除"))
}
